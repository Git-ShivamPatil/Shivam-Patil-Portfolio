/**
 * P13 — the secret registry: what every secret is, whether it is set, and how
 * to keep it out of a log.
 *
 * Split from vault.ts for one hard constraint. `instrumentation.ts` is bundled
 * for the Edge runtime as well as for Node, and it reaches the error reporter,
 * which reaches `scrub()`. Anything in that graph that so much as mentions
 * `node:fs` — even behind a dynamic import inside a branch that can never run
 * — makes the bundler warn about a module the Edge runtime does not have. So
 * this half is pure: no filesystem, no platform assumptions, safe anywhere.
 * vault.ts adds the mounted-file half on top and is imported only from Node.
 *
...
/**
 * Values supplied by something other than the environment — today, files
 * mounted by vault.ts. Held here rather than there so that `secret()` and
 * `scrub()` see them without this module having to know how they arrived.
 */
const injected = new Map<string, string>();

/** Called by vault.ts once it has read a mounted secret file. */
export function registerSecretValue(name: string, value: string): void {
  injected.set(name, value);
}

/** Whether a name was supplied out-of-band rather than from the environment. */
 ...
 * Read a secret.
 *
 * A mounted file wins over a plain environment variable when both are present,
 * because the file form is the more deliberate one: nobody mounts a secret file
 * by accident.
 */
export function secret(name: SecretName | (string & {})): string | undefined {
  // `|| undefined` rather than `??`: a blank value in a dashboard is how
  // "configured" and "working" quietly diverge, so an empty string counts as
  // unset. Parenthesised because mixing ?? with || is a syntax error.
  return injected.get(name) ?? (process.env[name] || undefined);
}
...
export function secretsReport(): SecretStatus[] {
  return (Object.keys(SECRETS) as SecretName[]).map((name) => {
    ...
    return {
      ...
      source: value ? (isInjected(name) ? "file" : "env") : "unset",
